...
describe('admin telemetry console', () => {
  it('renders the overview aggregation with token nulls and no cost columns for admins', async () => {
    ...
    mount('admin', fetcher)
    openTelemetryGroup()
    fireEvent.click(await screen.findByRole('button', { name: /总览运行与采集管道聚合/ }))
  ...
  it('keeps the telemetry group hidden from members and shows the server denial when forced', async () => {
    mount('member')
    expect(screen.queryByRole('button', { name: /AI Coding 可观测/ })).toBeNull()
  ...
  it('pages structured events with the opaque cursor and resets it when the server rejects it', async () => {
    ...
    const fetcher = telemetryFetcher([
      {
        ...
        respond: (url) => {
          calls += 1
          if (url.includes('cursor=') && calls === 2) return response({ code: 'INVALID_CURSOR', message: '游标无效' }, 400, 'INVALID_CURSOR')
          if (url.includes('cursor=')) return response({ project_id: 'project-alpha', items: [eventItem], next_cursor: null, has_more: false, retention_days: 90 })
    ...
    mount('admin', fetcher)
    openTelemetryGroup()
    fireEvent.click(await screen.findByRole('button', { name: /事件诊断结构化事件与数据缺口/ }))
