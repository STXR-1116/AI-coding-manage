...
export async function refreshServiceSession(baseUrl: string, refreshToken: string, fetcher: typeof fetch = fetch): Promise<RefreshedAccountSession | undefined> {
  ...
  let value: unknown
  try { value = await response.json() } catch { return undefined }
  ...
  const expiresIn = typeof value.expires_in === 'number' && Number.isFinite(value.expires_in) && value.expires_in > 0 ? value.expires_in : undefined
  ...
  if (accessToken === undefined || nextRefreshToken === undefined || expiresIn === undefined || mustChangePassword === undefined || role === undefined) return undefined
